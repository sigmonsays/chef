...
class Chef
  class Provider
    class Group
      class Opensolaris < Chef::Provider::Group
	...
	# this is the only method that changed from original provider
        def modify_group_members
          unless @new_resource.members.empty?
            if(@new_resource.append)
              @new_resource.members.each do |member|
                Chef::Log.debug("#{@new_resource}: appending member #{member} to group #{@new_resource.group_name}")
                run_command(:command => "usermod -a -G #{@new_resource.group_name} #{member}")
              end
            else
              Chef::Log.debug("#{@new_resource}: setting group members to #{@new_resource.members.join(', ')}")
              # run_command(:command => "usermod -G #{@new_resource.members.join(',')} #{@new_resource.group_name}")
	      raise Chef::Exceptions::Group, "set group members not supported on this platform"
            end
          else
